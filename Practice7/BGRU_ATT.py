...
class BGRU_ATT(pl.LightningModule):
    def __init__(self, input_size, hidden_size, classes):
        ...
        self.bgru = nn.GRU(self.hparams.input_size, self.hparams.hidden_size, batch_first=True, bidirectional=True)

        # Attention layer
        self.attention = nn.Linear(2 * self.hparams.hidden_size, 1)

        # Fully connected layer
        self.fc = nn.Linear(2 * self.hparams.hidden_size, self.hparams.classes)

        self.criterion = nn.CrossEntropyLoss()

    def forward(self, x):
        h0 = torch.zeros(2, x.size(0), self.hparams.hidden_size).to(self.device)
        out, _ = self.bgru(x, h0)
